 ...
 *
 * ── AND IT AUDITS THE LABORATORY, NOT THE PRODUCT ────────────────────────────────────
 * Every page this file loads is a `docs/3d/eN/live.html`. Nothing here has ever loaded `apps/web`, where the
 * eight relief surfaces ship — so "all N environments degrade to a readable flat surface" has always been a
 * statement about nine harnesses. The generated README now says so in its own SCOPE section, because a reader
 * reasonably assumes the programme's audit covers the programme's output.
 *
 * The app half is `scripts/3d-audit-app.mjs`, writing `docs/3d/APP_SWEEP.md`. Kept as a separate driver and a
 * separate output file for three reasons, argued at length in that file's header: the two page shapes need
 * different driving (a query-string control surface versus a seeded session, a replaced network and a button),
 * their prerequisites differ (a built harness bundle versus a Vite dev server) so one failing must not take the
