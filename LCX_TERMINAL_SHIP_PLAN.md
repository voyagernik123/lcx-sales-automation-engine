...
| T1 #9 | Sign-out leaves the read cache on disk — the page navigates away before the IndexedDB clear commits. Namespaced per operator so **not** inheritable, but `LCXOS_QUICKSTART.md` claimed it was cleared | 1h |
...
**Gate.** A scripted pass, on the packaged app, asserting: every modal in the app closes on one Escape and appears in `?`’s Escape section; every ranked table takes `↑↓`/`Home`/`End` and is one tab stop; every value served from cache carries an age affordance; and `LCXOS_QUICKSTART.md`’s cache claim is either true or corrected. A test asserts the dismiss-stack registration is **complete** — enumerate the modals, not spot-check them, or #15 recurs the next time someone adds one.
