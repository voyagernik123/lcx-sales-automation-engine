...
| POST-PROGRAM PENDING PASS | **LIVE both surfaces** (Pages: `Idempotency-Key` in the entry, `content-security-policy` header served with both inline hashes; Render deployment 6216384073 `success`, API up on the Node 22 image) · **desktop 0.3.0 building** | 4aedeaa | §5 "POST-PROGRAM PENDING PASS" | every open engineering item outside GPS closed with evidence or by a recorded decision; CI red-since-S3 fixed; CSP live; Idempotency-Key sent; Node 22; v0.3.0; 85 files, gate clean on run 3 (runs 1–2 each red on a true ratchet, fixed) |
