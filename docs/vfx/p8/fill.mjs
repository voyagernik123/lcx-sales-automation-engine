// P5 fill: P7 → P8 aggregates, the desk-route table (with figures = "did content render"), hero panels held, fade hits, redraw.
...
console.log(`  THE SWEEP (docs/instrument/audit/production-p5; 79 routes × 2 themes, fixtures incl. heroes and desk charts, one-page-load pairs,
    ...
    ${line('dark')}
    ${line('light')}
    DESK ROUTES (P7 → P8; "figures" says whether content rendered — with the fixtures the desks are opaque cards over the plate):
${desk}
...
// P8 · MEMORY CEILINGS: the JS heap sampled at rest per capture (Chromium performance.memory), the max per theme and where.
{
  const caps = (b.routes ?? []).flatMap((r) => Object.entries(r.runtime ?? {}).filter(([, t]) => t && t.reached === 'REACHED' && t.memory).map(([th, t]) => ({ path: r.path, th, mb: t.memory.usedJSHeapMB })));
  if (caps.length) {
    const worst = caps.reduce((m, c) => (c.mb > m.mb ? c : m), caps[0]);
    const med = caps.map((c) => c.mb).sort((a, b) => a - b)[Math.floor(caps.length / 2)];
    console.log(`    MEMORY (P8): JS heap at rest over ${caps.length} captures · median ${med} MB · max ${worst.mb} MB on ${worst.path} ${worst.th}`);
  } else console.log('    MEMORY (P8): no heap samples in this record');
}
