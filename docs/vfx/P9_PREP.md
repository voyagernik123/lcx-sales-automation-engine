...
## Publisher facts (read 2026-09-04)
- `publish-release.mjs` dies without `src-tauri/target/release/bundle` (run `build:dmg` first), on a STALE BUILD (tauri.conf.json version ≠ the built .app's), on ≠ 1 DMG in bundle/dmg, on a non-production API origin in apps/web/dist/assets, on a missing/stale `.sig`, and on the download page's `LCXOS_DMG_MB` disagreeing with the DMG; the published DMG name is `LCXOS-macOS-arm64.dmg`.
- Host: since Xcode 26.6 (2026-09-04 15:41) `/usr/bin/{git,python3,cc}` demand the licence; `export PATH=/Library/Developer/CommandLineTools/usr/bin:$PATH` first — cc/clang (Apple clang 21) and cargo 1.97 resolve there; `xcrun` itself still needs the licence if anything calls it.
- Tauri CLI binary strings: `xcrun` appears only for notarytool/stapler (notarization — not done here) and simctl (iOS); the macOS bundle uses `codesign` and `hdiutil` (system binaries). The v0.5.0 build is reachable with the CLT PATH and no licence acceptance.

- Rust link step calls `xcrun --show-sdk-path`; under the licence block set `DEVELOPER_DIR=/Library/Developer/CommandLineTools` and `SDKROOT=…/CommandLineTools/SDKs/MacOSX.sdk` (no sudo). The scripts do.
