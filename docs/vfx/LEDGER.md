...
**NEXT ACTION (2026-09-02 22:30):** P5 is LIVE (a395018). P6 pre-work is committed (f3a24eb) and pushed with this ledger. Begin P6 proper per docs/vfx/P6_PREP.md — after the Fable weekly reset (~2026-09-03 17:30 local): loader is done; next is wiring (ForgeBackdrop swaps disc/ring/plinth MeshBuffers for the glb meshes, still fallback unchanged), the asset under public/objects with MAX_PASSTHROUGH_KB 1024 → 1152 stated in the body, sidecar + oneObject.test, brand hex from bytes, sweep on /select-operator + /lcxos, the watch fixture (§4) added to the harness first so the captures lose the failure string.
