...
| P8 HARDENING | **COMMITTED (this commit; verify-live pending → then LIVE)** — context loss said and recovered, frame budget measured on the M1 GPU at 2×, glass ceiling re-measured and the glow cores brought under it, verify-app-renders over every route × DPR × width, memory sampled, print pins | | | |
