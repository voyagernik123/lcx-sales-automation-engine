...
| P6 THE OBJECTS (glTF) | **COMMITTED (this commit; verify-live pending → then LIVE)** — forge.glb 160,520 B live on /select + /lcxos (still's camera), 8 machined room markers, plate chamfer (walls rendered for the first time), loader 5 tests, sidecar + brand-hex-from-bytes tests; sweep: dark visible 72 → 74, light 77 → 79 of 79, desks ± 1, redraw median 4 ms | | | |
| P7 LIVENESS | PENDING | | | |
| P8 HARDENING | PENDING | | | |
| P9 PRODUCTION GATE + RELEASE | PENDING | | | |

**NEXT ACTION (2026-09-04, P6 close-out — model-agnostic):** (1) final gate `npm run ci-check` from the root → paste the five totals + perf line into `docs/vfx/p6/commit-msg.txt` at {{GATE}} (and the passthrough after-figure at {{PASS_AFTER}} if still open). (2) `git add packages/gl/src/env/gltf.ts packages/gl/src/env/gltf.test.ts packages/gl/src/env/stageScene.ts packages/gl/src/env/slabGeometry.test.ts packages/gl/src/index.ts scripts/blender/export_gltf.py scripts/instrument-audit.mjs scripts/instrument-fixtures.mjs apps/web/public/objects/forge.glb apps/web/public/objects/forge.glb.render.json apps/web/scripts/check-bundle.mjs apps/web/e2e/framebudget.spec.ts apps/web/src/components/brand apps/web/src/components/stage/Stage.tsx apps/web/src/pages/Launch.tsx apps/web/src/lib/__tests__/oneObject.test.ts apps/web/src/components/__tests__/glContextBudget.test.ts apps/web/src/components/__tests__/reliefTheme.test.tsx apps/web/src/components/__tests__/reliefRedrawRatchet.test.ts docs/vfx/LEDGER.md docs/vfx/GALLERY.md docs/vfx/gallery docs/vfx/p6 docs/vfx/P6_PREP.md docs/vfx/P7_PREP.md docs/vfx/P8_PREP.md docs/instrument/audit/production-p6` — then `git status --short` for anything P6 touched that this list misses. (3) flip this row to COMMITTED, `git commit -F docs/vfx/p6/commit-msg.txt`, `git push lcx-sales dev:main`. (4) `bash scripts/verify-live.sh <sha> --js 'objects/forge.glb' --lazy-js 'data-forge'` + `curl -sI https://lcx-sales-automation-engine.pages.dev/objects/forge.glb` (200, 160520 bytes) + open the CI run. (5) flip to LIVE; memory. (6) P7 per docs/vfx/P7_PREP.md (the staged patch scripts in the session scratchpad are session-only; the PREP carries the design).
