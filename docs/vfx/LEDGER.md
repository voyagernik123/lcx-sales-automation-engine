...
| P7 LIVENESS | **COMMITTED (this commit; verify-live pending → then LIVE)** — rooms lit in rank order on the arrival sweep (roomLitAt, refs for the P3 stale closure, bloom once); the instrument's controlled clock measures the sweep as one bounded sequence | | | |
