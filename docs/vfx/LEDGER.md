...
- **2026-09-04 — PRODUCTION REVIEW COMMIT 0af95c9** pushed to main. Root gate for it: `npm run ci-check` EXIT 0 — vitest stages: 1997 passed (1997);373 passed (373);3537 passed | 14 skipped (3551);2825 passed (2825);20 passed (20); shared→api→web emit builds and the web build green. The commit message's "Gate:" line carries a literal placeholder: the substitution that fills it broke on the `|` in "3537 passed | 14 skipped" and the commit went out before I read it. The gate result is recorded HERE and in the release commit that follows; the commit itself is not rewritten (main is never rewritten).
- **2026-09-04 — 0af95c9 VERIFIED LIVE on both surfaces** (verify-live: Render deployment success, both Pages needles present). Seen in the seated browser afterwards: the switcher paints above the page; the watch line reads deadlines, not a bind error; `/win-loss` renders in ~10 s with the deterministic narrative; the orrery plate is collapsed; Origination's Promote/Refuse are whole; the CSP header carries the beacon origin. **Three things the first deploy showed:** the coach mark under the top bar sat on the page subtitle → a one-line strip above the status bar; the US globe plate was a 30 px one-word column (a flipped plate laid out in the sliver right of its anchor before its −100 % translate) → flipped plates anchored by `right`, and plate heights now count WRAPPED lines; the beacon, once loadable, reported to `cloudflareinsights.com/cdn-cgi/rum` and `connect-src` blocked it → allowed. Second-pass commit follows its own root gate.
- **2026-09-04 — DESKTOP v0.5.1 PUBLISHED** (tag v0.5.1 on voyagernik123/lcx-terminal-releases). Built twice from the a5df264 tree with the six homes at 0.5.1; DMG 4,463,630 bytes = 4.5 MB (LCXOS_DMG_MB stays 4.5; the history line carries the first build's 4_463_623). release:dry then release; the publisher fetched the DMG and latest.json anonymously (HTTP 200, endpoint serves 0.5.1 on the first attempt); CDN read-back: version 0.5.1, signature identical to the local .app.tar.gz.sig. Published only after a5df264 was live on both surfaces and CI green on both jobs (run 33884044810). Release commit follows its own root gate.
- **2026-09-04 — RELEASE COMMIT 3a297dd LIVE + CI GREEN.** verify-live: Render deployment success, Pages carries `0.5.1` in the lazy chunks and the download page reads "0.5.1 · 4.5 MB · APPLE SILICON"; CI run 33886126412 GREEN on both jobs (gate; Playwright). The published DMG is byte-identical to the local build (`cmp`), installed to /Applications/LCXOS.app (0.5.1, binary 10,747,200 B) and launched — macOS then asked for the login keychain password for the "com.lcx.terminal" item (an unsigned build re-asks per binary); that prompt is Nik's alone. **The production review is closed: 0af95c9 → a5df264 → 3a297dd, all live, all CI green.**
- **2026-09-14 — THE STAGE REFUSES A SOFTWARE RASTERISER (`data-stage="refused:SOFTWARE_RASTERISER"`).** Found through CI, not a screenshot: `tablestops.spec` and `hints.spec` red on 4e24aa1 and 72d0733 with `[data-list-row]` / `[data-hint-tag]` "not found" at 6 s — the same class a5df264 and 08682a5 had carried as 3–4 flaky since the Stage landed, and 82871c6's green run was the fast-runner case (3.9 min against 5.9–6.2). Measured on a cold `vite` dev server with the seat seeded (scratch `measure-first-hit.mjs`, `measure-ab.mjs`, `measure-profile.mjs`): headless Chromium IS SwiftShader; the shell paints at 0.3 s, the rows at 1.2–1.7 s, `f`→chip 1.34 s. Block the Stage module and the rows arrive at 0.32–0.39 s, the chips at 0.13 s; block `forge.glb` and nothing changes. The CPU profile put 1.4 s in "(program)" — the compositor waiting on the GPU process, not JS; WebGL calls on the main thread totalled 22 ms. On ANGLE Metal (M1, headed) the same setup costs ~0.1 s (shell 401–458 → rows 523–540 ms), so the room stays on hardware. Fix: `isKnownSoftwareRasteriser` in `useQualityTier.ts` — POSITIVE identification only (unknown stays hardware, the opposite asymmetry to the measurement predicate; both on one shared pattern, WARP added) — and `Stage.tsx` hands the context back before compiling a shader, hiding the canvas (`alpha:false` composites an undrawn buffer black). `__LCX_GL_SOFTWARE_OK` keeps the room for the four headless harnesses (`instrument-audit`, `measure-stage-luminance`, `measure-frame-budget`, `3d-audit-app`), which set it. `tablestops.spec` now opens through `goToDesk`. After the change, under SwiftShader: rows 0.32–0.64 s, chips 0.14 s. **NOT done, on purpose:** the Forge (sign-in) still renders live on software GL — its still is the fallback and the front-door pixel ratchet photographs the live canvas under headless; refusing it there is a separate decision with baselines attached.
- **2026-09-14 — STAGE-ON-SOFTWARE-GL COMMIT** root gate `npm run ci-check` EXIT=0 — vitest stages:       Tests  1997 passed (1997);      Tests  380 passed (380);      Tests  3540 passed | 14 skipped (3554);      Tests  2839 passed (2839);      Tests  20 passed (20). Local evidence run (one worker, fresh server): tablestops + hints + practice + manual, 36 passed. Headed M1 (Metal): `data-stage="drawn"`, tier full, 8 pucks.
- **2026-09-14 — 14a78f1 LIVE + CI GREEN ON BOTH JOBS** (run 34866395041). verify-live by content: Pages carries `__LCX_GL_SOFTWARE_OK` and `refused:SOFTWARE_RASTERISER` in the eager index chunk (the `--lazy-js` probe could never see it — a needle in the shell chunk needs `--js`); Render deployment 14a78f1 success. Playwright: **77 passed, 2 skipped, 0 flaky, 1.6 min** — against 71 passed / 6 flaky / 1 failed / 5.9 min on 72d0733 and 68 / 8 / 1 / 6.2 min on 4e24aa1. Live desk on prod, seat seeded, `/v1` aborted: headless (SwiftShader) `data-stage="refused:SOFTWARE_RASTERISER"`, canvas hidden, rows 4 ms after the shell; headed M1 (Metal) `data-stage="drawn"`, 8 pucks, rows 86 ms after the shell. Nothing an operator on hardware sees has changed.
