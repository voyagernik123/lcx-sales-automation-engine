# THE APP SWEEP — status: **7 of 8 relief surfaces reached, 5 with findings**
...
| surface | print sheet on the page | canvases shown on screen → in print | inside `.br-no-print` | readable figures, relief off → relief on, printing | toggle prints |
|---|---|---|---|---|---|
| **E8** | NO | 1 → 1 | 0 | 0 → 0 | 0 |
| **E4** | NO | 1 → 1 | 0 | 1 → 0 **(lost)** | **1** |
| **E3** | NO | 1 → 1 | 0 | 1 → 0 **(lost)** | **1** |
| **E2** | NO | 1 → 1 | 0 | 1 → 0 **(lost)** | **1** |
| **E6** | NO | 1 → 1 | 0 | 1 → 0 **(lost)** | **1** |
| **E1** | yes | 2 → 0 | 1 | 2 → 2 | 0 |
| **E5** | yes | 2 → 0 | 1 | 2 → 2 | 0 |
| **E7** | yes | — | — | — | — |

"Readable figures" counts data tables and SVGs that contain text — the two things these wrappers swap out and
the two things that survive to paper. It is compared **relief off against relief on**, because a drop is the
finding and an absence is not: the sign-in screen has no flat data figure to lose and never claimed one, and an
absence test flagged it.
...
**E4 OntologyOrrery** — `src/components/geometry/OntologyOrrery.tsx`, on `/ontology`
- opening the relief removes 0 table(s) and 1 text-bearing SVG(s) from the document, and no print rule anywhere in apps/web puts them back — so ⌘P with the relief open prints a bitmap where the readable figure was (reliefPrintPath.test.tsx:44-48, item 2, now measured rather than stated as unverified)

**E3 PipelineRelief** — `src/components/geometry/PipelineRelief.tsx`, on `/bd-pipeline`
- opening the relief removes 1 table(s) and 0 text-bearing SVG(s) from the document, and no print rule anywhere in apps/web puts them back — so ⌘P with the relief open prints a bitmap where the readable figure was (reliefPrintPath.test.tsx:44-48, item 2, now measured rather than stated as unverified)
