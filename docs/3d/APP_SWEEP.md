# THE APP SWEEP — status: **7 of 8 relief surfaces reached, 4 with findings**
...
|---|---|---|---|---|---|
| **E8** ForgeBackdrop | `/select` | yes | the sign-in route; no seat, no fixture, no toggle — it mounts and runs | 0 of 1 canvases | 0 |
| **E4** OntologyOrrery | `/ontology` | yes | static ontology data — reachable with the network dead | 1 of 1 canvases (`full`) | 0 |
| **E3** PipelineRelief | `/bd-pipeline` | yes | one stubbed lead page, the same shape e2e/populated.spec.ts uses | 1 of 1 canvases (`full`) | **1** |
| **E2** GlobeRelief | `/market-map` | yes | one stubbed map page | 0 of 1 canvases | 0 |
| **E6** VaultRelief | `/audit-log` | yes | stubbed audit page PLUS a filter change, because the page's first audit read is dispatched dead — see the dead-read column | 1 of 1 canvases (`full`) | **1** |
| **E1** DeckRelief | `/command-deck` | yes | six stubbed command endpoints; the page mounts the house print sheet | 1 of 2 canvases (`full`) | 0 |
| **E5** SurfaceRelief | `/command-deck` | yes | the same deck, plus POST /v1/command/engines/lp-rescore for the ranking it draws | 1 of 2 canvases (`full`) | 0 |
...
- **E7 StormRelief** (`/marketing/crisis`) — `TOGGLE_DISABLED`: No calendar — NO_FORWARD_RISK_FEEDNo forward risk feed reaches this desk.
