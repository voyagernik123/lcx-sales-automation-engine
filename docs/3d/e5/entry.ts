...
if (MESH_ON && surfMesh) {
  draws.push({
     ...
     *
     * DIELECTRIC, and the reason is order rather than hex. A metal has no diffuse lobe, so its
     * colour arrives only through the specular F0 and #2C6BFF becomes a blue-tinted mirror of the
     * sky — which means the surface's rendered value tracks the ENVIRONMENT and the view angle
     * instead of the height it encodes, and two cells at different heights can read identical.
     * That breaks §6 rule 5 as amended 2026-08-14 ("ORDER SURVIVES THE PIPELINE"). This comment
     * used to cite the rule's old wording, "keeps the hex exact"; no lit material keeps a hex —
     * radiance is base colour × illumination, measured 46–88 ΔE on this path
     * (docs/3d/brand-fidelity.json) — so the old citation argued for the right material from a
     * property the surface never had.
     *
     * The anisotropy runs along the tangent the heightfield built, which is the +x grid
