...
/* Present through the pipeline's OWN tone curve. This said a second tone map here "would fork the one
   thing in this renderer whose output is verified brand-exact" — nothing in this renderer is verified
   brand-exact and nothing ever was; measured, the single curve already lands #2c6bff at #2c68dc
   (docs/3d/brand-fidelity.json). What a second one would fork is the MEASURED RECORD:
   packages/gl/src/look/look.test.ts asserts the composite is the only caller of lcxToneMap, and
   brandPixel.test.ts reproduces GPU bytes from exactly one application of it. */
...
const draws: LitDraw[] = [
  ...
  ...placed.map((p): LitDraw => ({
    ...
    /*
     * NEAR-DIELECTRIC, and that is a data constraint before it is a taste one. A metal has no
     * diffuse lobe: its colour arrives only through the specular F0, so pushing metalness up turns
     * #2C6BFF into a blue-tinted mirror of the sky — the panel's rendered value would then track the
     * environment and the view angle instead of the risk it encodes, and two panels at different
     * levels could read identical. That is what §6 rule 5 as amended 2026-08-14 forbids: ORDER
     * SURVIVES THE PIPELINE.
     *
     * This used to say "§6 rule 5 says the hex stays exact", which was the rule's old wording and
     * was false of any lit material — radiance is base colour × illumination, and on this path every
     * palette colour measures 46–88 ΔE from its hex (docs/3d/brand-fidelity.json). The material
     * choice is unchanged; only the reason for it is now one that holds. Dielectric with a faint
     * sheen — glass-fronted displays, which is what they are.
     */
...
function frame() {
  ...
  lit.draw({
    ...
    ambientGain: 1.05, lightVP, shadow, shadowStrength: 0.92, shadowTaps: Q.shadowTaps, shadowBaseline: 1536, draws,
