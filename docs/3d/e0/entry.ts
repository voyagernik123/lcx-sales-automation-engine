...
/* Present the HDR target through the pipeline's OWN tone curve. This said a second tone map here
   "would fork the one thing in this renderer whose output is verified brand-exact" — nothing in this
   renderer is verified brand-exact and nothing ever was; measured, the single curve already lands
   #2c6bff at #2c68dc, ΔE76 18.3 (docs/3d/brand-fidelity.json). What a second one would fork is the
   MEASURED RECORD: packages/gl/src/look/look.test.ts asserts the composite is the only caller of
   lcxToneMap, and brandPixel.test.ts reproduces GPU bytes from exactly one application of it. */
