# THE THEME PASS — status: **7 of 8 surfaces captured in both themes, 1 measurably WORSE IN LIGHT, 1 WITHHELD**

<!-- GENERATED by scripts/3d-audit-app.mjs. Do not edit: run `APP_SWEEP_THEME_ONLY=1 node scripts/3d-audit-app.mjs`. -->

Swept 2026-08-16. **This file is output, not prose.** It is written separately from `docs/3d/APP_SWEEP.md` for
...
**The tree this run swept: `cc71d6f2a92f`** over 512 source files under `apps/web/src`,
...
- **SCENERY**, per theme: `ground`, `structure`, `plate`, `rule`, `skyHorizon`, `skyZenith`
...
| **E1** | `#2C6BFF` | yes | `brand` | 0.0 | a palette entry, but inside the floor of a data colour — the palette-level pair above, not this surface's doing |
...
| **E3** PipelineRelief | light | yes | 224.3 | **20.22** | 140→246 | 85 | 126 / 129 | **0.00** | — | 224.3 | **—** | — | 1.50 |
...
| **E1** DeckRelief | dark | yes | 51.5 | **27.22** | 15→86 | 24 | 31 / 31 | **0.00** | — | 51.5 | **—** | — | 0.00 |
| **E1** DeckRelief | light | yes | 205.3 | **22.00** | 154→231 | 30 | 28 / 29 | **0.00** | — | 205.3 | **—** | — | 0.00 |
...
| **E3** PipelineRelief | light | 0.81% → **0.00%** | 2.52:1 → **—** | the old floor was counting lit scenery as marks |
...
| **E1** DeckRelief | dark | 0.00% → **0.00%** | — → **—** | unchanged |
| **E1** DeckRelief | light | 0.00% → **0.00%** | — → **—** | unchanged |
...
|---|---|---|
| **E8** ForgeBackdrop | dark | below the achromatic ceiling 98.83% · **brand** 0.60% · **brandDeep** 0.28% · rule 0.20% · skyHorizon 0.05% |
| **E8** ForgeBackdrop | light | below the achromatic ceiling 98.82% · rule (palette) 0.38% · **brand** 0.34% · plate (palette) 0.18% · **refusal** 0.16% |
| **E4** OntologyOrrery | dark | below the achromatic ceiling 95.37% · **brand** 2.14% · rule 0.74% · **brandBright** 0.64% · plate 0.44% |
| **E4** OntologyOrrery | light | below the achromatic ceiling 95.74% · **brand** 2.07% · **refusal** 1.16% · **brandBright** 0.57% · rule (palette) 0.17% |
| **E3** PipelineRelief | dark | below the achromatic ceiling 98.50% · rule 0.71% · **brandDeep** 0.70% · structure 0.03% · skyHorizon 0.03% |
| **E3** PipelineRelief | light | below the achromatic ceiling 98.50% · rule (palette) 1.49% · plate (palette) 0.01% |
| **E2** GlobeRelief | dark | below the achromatic ceiling 81.13% · rule 7.30% · **brand** 2.10% · **brandBright** 2.07% · skyHorizon 1.89% |
...
| **E1** DeckRelief | dark | below the achromatic ceiling 100.00% |
| **E1** DeckRelief | light | below the achromatic ceiling 100.00% |
...
| **E1** DeckRelief | 81% | 108% | — | — | 0% | 0% | holds up · no data marks in either theme |
...
- WORSE IN LIGHT — 0.72% of the dark buffer is attributed to a data colour and **none** of the light buffer is, so the data marks are not separable from the scenery on a white page

## Other findings

**E1 DeckRelief** — `/command-deck`
- NO DATA MARKS IN EITHER THEME — not one pixel of this surface's buffer is attributed to a data colour in either theme, so the data:scenery contrast column is empty for both rows. Stated rather than left as a row of dashes: this is not a light-theme finding, and it is not a pass either
