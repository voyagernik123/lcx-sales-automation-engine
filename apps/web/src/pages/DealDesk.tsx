          ...
          <Section title="Partners & referrals" icon={<Briefcase size={15} className="text-emerald-600" />} onRefresh={() => void loadPartners()}>
            <div className="mb-3 flex gap-2">
              <input
                ...
                className="flex-1 rounded-lg border border-line bg-card px-2.5 py-1.5 text-xs text-navy focus-ring"
              />
              <input
                value={newPartner.commissionPct}
                onChange={(e) => setNewPartner((p) => ({ ...p, commissionPct: e.target.value }))}
                placeholder="%"
                className="w-16 rounded-lg border border-line bg-card px-2 py-1.5 text-xs text-navy focus-ring"
              />
