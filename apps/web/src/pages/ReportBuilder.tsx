  ...
  return (
    <div className="mx-auto max-w-3xl space-y-4 p-4">
      <h1 className="flex items-center gap-2 text-lg font-bold">
        <Table2 size={18} /> Report Builder
      </h1>
      <p className="text-[11px] text-grey">
        Ad-hoc aggregations over an allowlisted set of entities and columns — pick what to count/sum and how to group.
      </p>
      ...
      <div className="flex flex-wrap items-end gap-3 rounded-lg border border-line bg-card p-3">
        <label className="text-[11px]">
          ...
          <select value={entity} onChange={(e) => { setEntity(e.target.value); setGroupBy(''); }} className="rounded border border-line px-2 py-1">
            {schema.map((s) => <option key={s.entity} value={s.entity}>{s.entity}</option>)}
          </select>
        </label>
        <label className="text-[11px]">
          <div className="mb-1 font-bold text-grey">Group by</div>
          <select value={groupBy} onChange={(e) => setGroupBy(e.target.value)} className="rounded border border-line px-2 py-1">
        ...
        <label className="text-[11px]">
          ...
          <select value={metric} onChange={(e) => setMetric(e.target.value)} className="rounded border border-line px-2 py-1">
        ...
        <button
          ...
          className="inline-flex items-center gap-1 rounded bg-indigo-600 px-3 py-1.5 text-[11px] font-bold text-white hover:bg-indigo-700 disabled:opacity-50"
        >
          ...
          <div className="space-y-1">
            {result.rows.map((row, i) => (
              <div key={i} className="flex items-center gap-2 text-[11px]">
                ...
                <div className="h-3 flex-1 rounded bg-slate-100 dark:bg-slate-800 overflow-hidden">
            ...
            {result.rows.length === 0 && <p className="text-[11px] text-grey">No rows.</p>}
          </div>
        </div>
      )}
    </div>
