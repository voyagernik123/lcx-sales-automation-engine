    ...
    <div id={id} className="rounded-lg border border-line/70 bg-card shadow-card overflow-hidden">
      ...
      <div className={compact ? 'p-2 space-y-1.5' : 'p-3 space-y-2'}>
        ...
        {limit != null && trail.length > shown.length && (
          <p className="text-micro text-grey">+ {trail.length - shown.length} more factor(s) on the full dossier</p>
        )}
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded border border-line/70 px-2 py-1.5">
      <div className="flex items-center gap-2">
        <span className="text-micro font-semibold text-navy truncate">{reason.factor}</span>
        <div className="ml-auto flex items-center gap-1.5 shrink-0">
          {capped ? (
            <span className="rounded bg-slate-100 dark:bg-slate-800 px-1 py-0.5 text-micro font-semibold text-grey">cap</span>
          ) : (
            <div
          ...
          <span className="font-mono num-tabular text-micro font-bold text-navy">
            {reason.points}{reason.max > 0 ? `/${reason.max}` : ''}
          </span>
        </div>
      </div>
      {reason.note && <p className="mt-0.5 text-micro text-grey leading-snug">{reason.note}</p>}
    </div>
  );
}
