...
export const Select = forwardRef<HTMLSelectElement, SelectProps>(({ label, error, options, className, id, ...props }, ref) => {
  ...
  return (
    <div className="w-full">
      ...
      <select ref={ref} id={selectId} aria-invalid={!!error} className={clsx('block w-full rounded-md border bg-card px-3 py-2 text-sm shadow-sm focus-ring focus:ring-2 focus:ring-navy focus:border-navy disabled:opacity-50', error ? 'border-status-blocked' : 'border-line', className)} {...props}>
